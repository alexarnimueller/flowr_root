...
class SquaredModifier(ScoreModifier):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        return 1.0 - self.coefficient * torch.pow(self.target_value - x, 2)
...
class AbsoluteScoreModifier(ScoreModifier):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        return 1.0 - torch.abs(self.target_value - x)
...
class GaussianModifier(ScoreModifier):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        return torch.exp(-0.5 * torch.pow((x - self.mu) / self.sigma, 2.0))
...
class MinMaxGaussianModifier(ScoreModifier):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        if self.minimize:
            mod_x = torch.clamp(x, min=self.mu)
        else:
            mod_x = torch.clamp(x, max=self.mu)
...
class ClippedScoreModifier(ScoreModifier):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.clamp(y, min=self.low_score, max=self.high_score)
...
class SmoothClippedScoreModifier(ScoreModifier):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        return self.low_score + self.L / (1 + torch.exp(-self.k * (x - self.middle_x)))
