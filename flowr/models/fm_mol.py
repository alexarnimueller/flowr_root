    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.self_condition_mode == "residual":
            ...
            n_bond_types = (
                gen.ligand_dec.n_bond_types
                if hasattr(gen, "ligand_dec")
                and hasattr(gen.ligand_dec, "n_bond_types")
                else gen.hparams.get("n_edge_types", 5)
            )
        ...
        if training and torch.rand(1).item() < sc_prob:
            with torch.no_grad():
                ...
                if self.self_condition_mode == "stacking":
                    ...
                    # For residual mode: store raw softmaxed predictions + flag
                    cond_batch = {
                        "coords": cond_out["coords"],
                        "atomics": F.softmax(cond_out["atomics"], dim=-1),
                        "bonds": F.softmax(cond_out["bonds"], dim=-1),
    ...
    def forward(
        ...
        if cond_batch is not None:
            ...
            if (
                ...
                and hasattr(self.gen.ligand_dec, "sc_distance_edges")
