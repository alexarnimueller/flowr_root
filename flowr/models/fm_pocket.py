...
class LigandPocketCFM(pl.LightningModule):
    def __init__(
        ...
        self_condition_mode: str = "stacking",
        self_condition_prob: float = 0.5,
        sc_charges: bool = False,
        sc_distance_edges: bool = False,
        sc_schedule: str = "constant",
        sc_warmup_steps: int = 10000,
        sc_recycle_steps: int = 1,
        mask_fixed_discrete_loss: bool = False,
        **kwargs,
    ):
        super().__init__()
        ...
        self.self_condition_mode = self_condition_mode
        self.self_condition_prob = self_condition_prob
        self._inpaint_self_condition = inpaint_self_condition
        self.sc_charges = sc_charges
        self.sc_recycle_steps = sc_recycle_steps
        ...
        if sc_charges:
            self.sc_feature_keys.append("charges")

        # Adaptive SC schedule
        from flowr.models.self_conditioning import AdaptiveSCSchedule

        self.sc_schedule_obj = AdaptiveSCSchedule(
            mode=sc_schedule,
            start_prob=0.0,
            end_prob=self.self_condition_prob,
            warmup_steps=sc_warmup_steps,
        )
        ...
        self.use_is_fixed_embed = use_is_fixed_embed
        ...
        hparams = {
            ...
            "self_condition_mode": self_condition_mode,
            "self_condition_prob": self_condition_prob,
            "sc_charges": sc_charges,
            "sc_distance_edges": sc_distance_edges,
            "sc_schedule": sc_schedule,
            "sc_warmup_steps": sc_warmup_steps,
            "sc_recycle_steps": sc_recycle_steps,
        ...
        self.loss_computer = LossComputer(
            ...
            bond_strategy=bond_strategy,
            use_t_loss_weights=use_t_loss_weights,
            mask_fixed_discrete_loss=mask_fixed_discrete_loss,
        )
    ...
    def _prepare_self_condition_batch(
        ...
        if self.self_condition_mode == "stacking":
            ...
            if self.sc_charges:
                cond_batch["charges"] = torch.zeros_like(lig_interp["charges"])

        elif self.self_condition_mode == "residual":
            # Still need zero-filled cond_batch because the generator has self_cond=True
            # and its forward() validation requires cond_atomics/cond_bonds
            cond_batch = {
                "coords": lig_prior["coords"],
                "atomics": torch.zeros_like(lig_interp["atomics"]),
                "bonds": torch.zeros_like(lig_interp["bonds"]),
        ...
        sc_prob = self.sc_schedule_obj.get_prob(self.global_step) if training else 1.0
        if training and torch.rand(1).item() < sc_prob:
            with torch.no_grad():
                ...
                if self.self_condition_mode == "stacking":
                    ...
                    if self.sc_charges:
                        cond_batch["charges"] = F.softmax(cond_out["charges"], dim=-1)
    ...
    def forward(
        ...
        # Apply residual layer in residual mode.
        # Always execute to keep parameters in computation graph for DDP,
        # but only use the output when SC actually fired.
        if self.self_condition_mode == "residual" and cond_batch is not None:
            predicted_final = {
                "coords": cond_batch["coords"],
                "atomics": cond_batch["atomics"],
                "bonds": cond_batch["bonds"],
                "charges": cond_batch.get(
                    "charges", torch.zeros_like(batch["charges"])
                ),
                "mask": cond_batch.get("mask", batch["mask"]),
            }
            if self.add_feats and "hybridization" in cond_batch:
                predicted_final["hybridization"] = cond_batch["hybridization"]
            refined = self.residual_layer(batch, predicted_final)
            if cond_batch.get("_sc_fired", False):
                cond_batch = {
                    "coords": refined["coords"],
                    "atomics": refined["atomics"],
                    "bonds": refined["bonds"],
                }
                if self.add_feats:
                    cond_batch["hybridization"] = refined.get(
                        "hybridization", batch.get("hybridization")
                    )
                if self.sc_charges:
                    cond_batch["charges"] = predicted_final["charges"]
            else:
                # Zero contribution keeps residual layer params in the computation graph
                dummy = (
                    refined["coords"].sum()
                    + refined["atomics"].sum()
                    + refined["bonds"].sum()
                ) * 0.0
                cond_batch["coords"] = cond_batch["coords"] + dummy

        # Strip internal flags before passing to generator
        if cond_batch is not None:
            cond_batch = {k: v for k, v in cond_batch.items() if not k.startswith("_")}

        # Compute SC charges and distance edge features
        cond_charges = None
        cond_dist_edges = None
        if cond_batch is not None:
            if self.sc_charges:
                cond_charges = cond_batch.get("charges")
            if (
                hasattr(self.gen, "ligand_dec")
                and self.gen.ligand_dec.sc_distance_edges
            ):
                cond_dist_edges = self.gen.ligand_dec.dist_edge_sc(
                    cond_batch["coords"], mask
                )

        # Per-atom is_fixed conditioning
        is_fixed_node = None
        is_fixed_position = None
        if self.use_is_fixed_embed and inpaint_mask is not None:
            is_fixed_node = inpaint_mask.long()
            # For graph inpainting, positions are NOT fixed (only graph is fixed)
            is_fixed_position = torch.zeros_like(inpaint_mask, dtype=torch.long)
            # For fragment inpainting, positions ARE fixed for inpainted atoms
            if hasattr(self, "graph_inpainting") and not self.graph_inpainting:
                is_fixed_position = inpaint_mask.long()

        # Only pass is_fixed kwargs when the generator supports them
        gen_kwargs = {}
        if self.use_is_fixed_embed:
            gen_kwargs["is_fixed_node"] = is_fixed_node
            gen_kwargs["is_fixed_position"] = is_fixed_position
        ...
        if cond_batch is not None:
            out = self.gen(
                ...
                cond_charges=cond_charges,
                cond_dist_edges=cond_dist_edges,
    ...
    def _loss(self, data, interpolated, predicted, prior, pocket_data, times=None):
        ...
        return self.loss_computer.compute_losses(
            ...
            inpaint_mask=data.get("fragment_mask", None),
        )
    ...
    def _get_predictions(self, out):
        ...
        if self.sc_charges:
            cond_batch["charges"] = charge_probs
        if self.self_condition_mode == "residual":
            cond_batch["_sc_fired"] = True
            cond_batch["charges"] = charge_probs
            cond_batch["mask"] = out["mask"]
            if hybridization_probs is not None:
                cond_batch["hybridization"] = hybridization_probs
        predicted = {
            "coords": coords,
            "atomics": type_probs,
            "bonds": bond_probs,
    ...
    def _generate_for_confidence(
        ...
        if self.add_feats and "hybridization" in prior:
    ...
    def _generate(
        ...
        with torch.no_grad():
            ...
            for i, step_size in enumerate(step_sizes):
                ...
                # SC recycling: refine predictions with additional passes
                for _ in range(self.sc_recycle_steps - 1):
