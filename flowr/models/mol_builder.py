...
class MolBuilder:
    ...
    def _extract_mols(
        ...
        for idx in range(coords.size(0)):
            ...
            mol_bond_dists = (
                bond_dists[idx, :mol_atoms, :mol_atoms, :].cpu()
                if bond_dists is not None
                else None
            )
            mol_charge_dists = (
                charge_dists[idx, :mol_atoms, :].cpu()
                if charge_dists is not None
                else None
            )
            mol_hybridization_dists = (
                hybridization_dists[idx, :mol_atoms, :].cpu()
                if hybridization_dists is not None
                else None
            )
            mol_aromaticity_dists = (
                aromaticity_dists[idx, :mol_atoms, :].cpu()
                if aromaticity_dists is not None
                else None
            )

            # Remove virtual (NOATOM) atoms
            (
                mol_coords,
                mol_token_dists,
                mol_bond_dists,
    ...
    def _extract_pocket_or_lig(
        ...
        for idx in range(coords.size(0)):
            bool_mask = mask[idx].bool()
            mol_coords = coords[idx][bool_mask].cpu()
            mol_token_dists = (
                atom_dists[idx][mask[idx]].cpu() if atom_dists is not None else None
            )
            ...
            mol_charge_dists = (
                charge_dists[idx][bool_mask].cpu() if charge_dists is not None else None
            )
            mol_hybridization_dists = (
                hybridization_dists[idx][bool_mask].cpu()
                if hybridization_dists is not None
                else None
            )
            mol_aromaticity_dists = (
                aromaticity_dists[idx][bool_mask].cpu()
                if aromaticity_dists is not None
                else None
            )

            # Remove virtual (NOATOM) atoms
            if mol_token_dists is not None:
                (
                    mol_coords,
                    mol_token_dists,
                    mol_bond_dists,
    ...
    def _remove_virtual_atoms(
        ...
        """Remove atoms predicted as NOATOM before building RDKit mol."""
        from flowr import constants

        if not self.vocab.contains(constants.NOATOM_TOKEN):
            return (
                coords,
                atom_dists,
                bond_dists,
        ...
        noatom_idx = self.vocab.token_idx_map[constants.NOATOM_TOKEN]
        pred_types = torch.argmax(atom_dists, dim=-1)
        real_mask = pred_types != noatom_idx
        ...
        atom_dists = atom_dists[real_mask]
        if bond_dists is not None:
            bond_dists = bond_dists[real_mask][:, real_mask]
        if charge_dists is not None:
            charge_dists = charge_dists[real_mask]
        if hybridization_dists is not None:
            hybridization_dists = hybridization_dists[real_mask]
        if aromaticity_dists is not None:
            aromaticity_dists = aromaticity_dists[real_mask]
