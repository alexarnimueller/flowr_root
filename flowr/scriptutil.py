...
def _build_vocab(virtual_nodes: bool = False):
    # Need to make sure PAD has index 0
    special_tokens = ["<PAD>"]
    tokens = special_tokens + constants.CORE_ATOMS
    if virtual_nodes:
        tokens = tokens + [constants.NOATOM_TOKEN]
...
def build_model(
    ...
    if backbone == "e3nn":
        from flowr.models.e3nn_compat import require_e3nn

        require_e3nn()
        from flowr.models.e3nn_generator import E3LigandGenerator

        e3nn_msg_channels = None
        if getattr(args, "e3nn_msg_channels", None):
            e3nn_msg_channels = [int(x) for x in args.e3nn_msg_channels.split(",")]

        # Use the fast SEMLA pocket encoder (same output format, GPU-friendly)
        ...
        gen = E3LigandGenerator(
            ...
            use_mean_flow_delta_t=getattr(args, "use_mean_flow_delta_t", False),
            pocket_enc=pocket_enc,
            lmax=getattr(args, "e3nn_lmax", 2),
            msg_channels=e3nn_msg_channels,
            cutoff=getattr(args, "e3nn_cutoff", 5.0),
            cross_cutoff=getattr(args, "e3nn_cross_cutoff", 10.0),
            hidden_channels=getattr(args, "e3nn_hidden_channels", 128),
        )
    else:
        pocket_enc = PocketEncoder(
            pocket_d_equi,
    ...
    if args.lr_schedule in ["cosine", "constant"]:
        warm_up_steps = 0 if args.warm_up_steps == 0 else get_warm_up_steps(train_steps)
    else:
        warm_up_steps = 0
    ...
    fm_model = CFM(
        ...
        self_condition_mode=getattr(args, "self_condition_mode", "stacking"),
        self_condition_prob=getattr(args, "self_condition_prob", 0.5),
        sc_charges=getattr(args, "sc_charges", False),
        sc_distance_edges=getattr(args, "sc_distance_edges", False),
        sc_schedule=getattr(args, "sc_schedule", "constant"),
        sc_warmup_steps=getattr(args, "sc_warmup_steps", 10000),
        sc_recycle_steps=getattr(args, "sc_recycle_steps", 1),
...
def load_model(
    ...
    hparams["self_condition_prob"] = hparams.get("self_condition_prob", 0.5)
    hparams["sc_charges"] = hparams.get("sc_charges", False)
    hparams["sc_distance_edges"] = hparams.get("sc_distance_edges", False)
    hparams["sc_schedule"] = hparams.get("sc_schedule", "constant")
    hparams["sc_warmup_steps"] = hparams.get("sc_warmup_steps", 10000)
    hparams["sc_recycle_steps"] = hparams.get("sc_recycle_steps", 1)
    ...
    vocab = _build_vocab(virtual_nodes=getattr(args, "virtual_atom_p", 0.0) > 0)
    ...
    if args.arch == "pocket":
        ...
        egnn_gen = LigandGenerator(
            ...
            sc_charges=hparams.get("sc_charges", False),
            sc_distance_edges=hparams.get("sc_distance_edges", False),
...
def build_mol_mean_flow_model(
    ...
    """Build a MeanFlow-enabled molecule generation model.

    Extends build_mol_model with MeanFlow-specific parameters.
    Uses LigandMeanFlowCFM instead of LigandCFM.
    """
    ...
    from flowr.models.fm_mol_mean import LigandMeanFlowCFM
    ...
    gen = LigandGenerator(
        ...
        coord_skip_connect=not args.no_coord_skip_connect,
        coord_update_every_n=getattr(args, "coord_update_every_n", None),
        use_mean_flow_delta_t=True,  # Enable delta_t embedding
        use_is_fixed_embed=getattr(args, "use_is_fixed_embed", False),
    )
    ...
    fm_model = LigandMeanFlowCFM(
        ...
        mean_flow_mf_proportion=getattr(args, "mean_flow_mf_proportion", 0.5),
        mean_flow_time_distribution=getattr(
            args, "mean_flow_time_distribution", "logit-normal"
        ),
        mean_flow_adaptive_weight_power=getattr(
            args, "mean_flow_adaptive_weight_power", 1.0
        ),
        mean_flow_adaptive_weight_eps=getattr(
            args, "mean_flow_adaptive_weight_eps", 0.01
        ),
        mean_flow_steps=getattr(args, "mean_flow_steps", 5),
        mean_flow_warmup_steps=getattr(args, "mean_flow_warmup_steps", 2000),
        mean_flow_warmup_ramp_steps=getattr(args, "mean_flow_warmup_ramp_steps", 5000),
        mean_flow_correction_clip=getattr(args, "mean_flow_correction_clip", 10.0),
        mean_flow_time_eps=getattr(args, "mean_flow_time_eps", 1e-3),
        mean_flow_coord_loss_weight=getattr(args, "mean_flow_coord_loss_weight", 1.0),
        mean_flow_discrete_loss_weight=getattr(
            args, "mean_flow_discrete_loss_weight", 0.5
        ),
        # Standard LigandCFM parameters
        add_feats=args.add_feats,
        vocab_hybridization=vocab_hybridization,
        vocab_aromatic=vocab_aromatic,
        weight_decay=args.weight_decay,
