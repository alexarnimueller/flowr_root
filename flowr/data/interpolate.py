...
class GeometricNoiseSampler(NoiseSampler):
    ...
    def _generate_harmonic_coordinates(
        ...
        """
        Generate coordinates using degree-scaled Laplacian eigendecomposition
        of the molecular graph (following mosaic's IncompleteHarmonicPrior).
        """
        eps = 1e-2
        try:
            ...
            # Filter out non-bonded pairs (bond_type == 0 means no bond)
            ...
            # Build adjacency matrix A from bond edges
            A = torch.zeros((n_atoms, n_atoms), dtype=torch.float32, device=device)
            A[edges[:, 0], edges[:, 1]] = 1.0
            A[edges[:, 1], edges[:, 0]] = 1.0

            # Degree-regularized Laplacian: L = D - A, with eps on diagonal
            D = A.sum(dim=1) + eps
            L = torch.diag(D) - A

            # Eigendecompose
            U, V = torch.linalg.eigh(L)

            # Sample: x = V @ (noise / sqrt(eigenvalues))
            std = (1.0 / U.sqrt()).unsqueeze(-1)  # [n_atoms, 1]
            noise = torch.randn(n_atoms, 3, device=device)
            coords = V @ (noise * std)

            # Scale by harmonic_prior_sigma and center
            coords = coords * self.harmonic_prior_sigma
...
class GeometricInterpolant(Interpolant):
    def __init__(
        ...
        harmonic_prior_sigma: float = 1.0,
        ...
        self.graph_inpainting_prob = graph_inpainting_prob
        self.skip_ot_for_graph_inpainting = skip_ot_for_graph_inpainting
        self.harmonic_prior_sigma = harmonic_prior_sigma
        self.prior_sampler.harmonic_prior_sigma = harmonic_prior_sigma
        self.sample_mol_sizes = sample_mol_sizes
        self.virtual_atom_p = virtual_atom_p
        ...
        if coord_noise_schedule == "constant_decay":
            ...
            self.noise_scheduler = GeometryDistortionNoiseSchedule(
                p_distort=0.5, t_distort=0.25, sigma_distort=coord_noise_std
            )
        else:
    ...
    def _append_virtual_nodes(self, mol: GeometricMol) -> GeometricMol:
        """Append virtual (NOATOM) atoms to a molecule during training.

        Virtual atoms are placed near the center of mass with Gaussian noise.
        They have NOATOM type, zero charges, and no bond edges (defaults to NOBOND=0 in adjacency).
        """
        n_real = mol.seq_length
        max_virtual = math.ceil(n_real * self.virtual_atom_p)
        if max_virtual == 0:
            return mol
        n_virtual = torch.randint(0, max_virtual + 1, (1,)).item()
        if n_virtual == 0:
            return mol

        # Coordinates: center of mass + Gaussian noise
        com = mol.coords.mean(dim=0, keepdim=True)
        virtual_coords = (
            com.expand(n_virtual, -1)
            + torch.randn(n_virtual, 3) * self.virtual_atom_noise_std
        )
        new_coords = torch.cat([mol.coords, virtual_coords], dim=0)

        # Atom types: one-hot NOATOM
        vocab_size = mol.atomics.size(-1)
        noatom_onehot = torch.zeros(n_virtual, vocab_size)
        noatom_onehot[:, self.noatom_index] = 1.0
        new_atomics = torch.cat([mol.atomics, noatom_onehot], dim=0)

        # Charges: PAD (index 0) one-hot
        if mol.charges.dim() == 2:
            charge_size = mol.charges.size(-1)
            virtual_charges = torch.zeros(n_virtual, charge_size)
            virtual_charges[:, 0] = 1.0  # PAD
        else:
            virtual_charges = torch.zeros(n_virtual, dtype=mol.charges.dtype)
        new_charges = torch.cat([mol.charges, virtual_charges], dim=0)

        # Bonds: no new edges for virtual atoms
        # Missing edges default to NOBOND (index 0) in adjacency matrix reconstruction
        new_bond_indices = mol.bond_indices
        new_bond_types = mol.bond_types

        # Hybridization: PAD for virtual atoms (if present)
        new_hybridization = None
        if mol.hybridization is not None:
            if mol.hybridization.dim() == 2:
                virtual_hybrid = torch.zeros(n_virtual, mol.hybridization.size(-1))
                virtual_hybrid[:, 0] = 1.0
            else:
                virtual_hybrid = torch.zeros(n_virtual, dtype=mol.hybridization.dtype)
            new_hybridization = torch.cat([mol.hybridization, virtual_hybrid], dim=0)

        # Is aromatic: PAD for virtual atoms (if present)
        new_is_aromatic = None
        if mol.is_aromatic is not None:
            if mol.is_aromatic.dim() == 2:
                virtual_aromatic = torch.zeros(n_virtual, mol.is_aromatic.size(-1))
                virtual_aromatic[:, 0] = 1.0
            else:
                virtual_aromatic = torch.zeros(n_virtual, dtype=mol.is_aromatic.dtype)
            new_is_aromatic = torch.cat([mol.is_aromatic, virtual_aromatic], dim=0)

        # Fragment mask: extend with zeros for virtual atoms (not part of any fragment)
        new_fragment_mask = None
        if mol.fragment_mask is not None:
            virtual_frag = torch.zeros(n_virtual, dtype=mol.fragment_mask.dtype)
            new_fragment_mask = torch.cat([mol.fragment_mask, virtual_frag], dim=0)

        # RDKit features: molecule-level descriptors (1D), not per-atom — pass through unchanged
        new_rdkit_feats_cont = mol.rdkit_feats_cont
        new_rdkit_feats_disc = mol.rdkit_feats_disc

        return mol._copy_with(
            coords=new_coords,
            atomics=new_atomics,
            ...
            is_aromatic=new_is_aromatic,
            fragment_mask=new_fragment_mask,
            rdkit_feats_cont=new_rdkit_feats_cont,
            rdkit_feats_disc=new_rdkit_feats_disc,
        )
    ...
    def _interpolate_standard(self, to_mols: list[GeometricMol]) -> _GeometricInterpT:
        ...
        # Apply virtual node augmentation during training
        if self.virtual_atom_p > 0 and not self.inference:
            to_mols = [self._append_virtual_nodes(mol) for mol in to_mols]
        ...
        # At inference with virtual nodes: add headroom for the model to decide atom count
        if self.virtual_atom_p > 0 and self.inference:
            mol_sizes = [
                int(math.ceil(s * (1 + self.virtual_atom_p))) for s in mol_sizes
            ]
        ...
        interp_mols = (
            ...
            if not (
                (self.sample_mol_sizes or self.virtual_atom_p > 0) and self.inference
            )
            else from_mols
        )
    ...
    def _training_inpainting(
        ...
        # Append virtual nodes to de_novo target molecules during training
        # (must happen after mode determination but before mol_sizes computation)
        if self.virtual_atom_p > 0:
            to_mols = [
                self._append_virtual_nodes(to_mol) if mode == "de_novo" else to_mol
                for to_mol, (mode, _mask, _is_local) in zip(to_mols, modes_and_masks)
            ]
        ...
        if self.permutation_alignment or self.rotation_alignment:
            ...
            from_mols = [
                (
                    from_mol
                    if (mode == "graph" and self.skip_ot_for_graph_inpainting)
                    else self._match_mols(from_mol, to_mol, mol_size=mol_size)
                )
                for from_mol, to_mol, mol_size, (mode, _mask, _is_local) in zip(
                    from_mols, to_mols, mol_sizes, modes_and_masks
                )
            ]
        else:
            # Sample prior molecules (standard size)
        ...
        for i in range(batch_size):
            if self.inference:
                ...
                if active_graph_modes:
                    weight_map = {
                        "de_novo": max(
                            0.0, 1.0 - self.graph_inpainting_prob - 0.25 - 0.25
                        ),
                        "graph": self.graph_inpainting_prob,
                        "local": 0.25,
                        "global": 0.25,
                    }
                else:
            ...
            if mode == "de_novo":
                ...
                if mask is None:
                    # No interactions available — fall back to de novo
                    print(
                        f"Warning: No interaction mask available for molecule {i}, falling back to de novo mode."
                    )
                    mode = "de_novo"
                    mask = torch.zeros(to_mols[i].seq_length, dtype=torch.bool)
            else:
...
class ComplexInterpolant(GeometricInterpolant):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            graph_inpainting_prob=graph_inpainting_prob,
            skip_ot_for_graph_inpainting=skip_ot_for_graph_inpainting,
            harmonic_prior_sigma=harmonic_prior_sigma,
            ...
            virtual_atom_p=virtual_atom_p,
            virtual_atom_noise_std=virtual_atom_noise_std,
            noatom_index=noatom_index,
        )
        if sample_mol_sizes:
    ...
    def interpolate(self, to_mols: list[PocketComplex]) -> _ComplexInterpT:
        ...
        # Resize to_interactions when virtual nodes enlarged the ligand
        if to_interactions and self.virtual_atom_p > 0:
            resized_to_interactions = []
            for i, interaction in enumerate(to_interactions):
                if interaction is not None:
                    n_to_lig = to_ligands[i].seq_length
                    n_orig_lig = interaction.shape[1]
                    if n_to_lig != n_orig_lig:
                        n_pocket = interaction.shape[0]
                        n_types = interaction.shape[2]
                        resized = torch.zeros(
                            n_pocket,
                            n_to_lig,
                            n_types,
                        ...
                        resized[:, :n_orig_lig, :] = interaction
                        resized_to_interactions.append(resized)
                    else:
                        resized_to_interactions.append(interaction)
                else:
                    resized_to_interactions.append(None)
            to_interactions = resized_to_interactions
        ...
        if self.flow_interactions:
            ...
            # When not flowing interactions, use to_interactions directly.
            # If ligand sizes were varied (sample_mol_sizes), resize the
            # interaction tensors to match from_ligand dimensions so that
            # from_systems / interp_systems carry consistently shaped tensors.
            if (
                (self.sample_mol_sizes or self.virtual_atom_p > 0)
                and to_interactions
                and self.inference
            ):
                from_interactions = []
                for i in range(batch_size):
                    n_from_lig = from_ligands[i].seq_length
                    n_to_lig = to_ligands[i].seq_length
                    if to_interactions[i] is not None and n_from_lig != n_to_lig:
                        n_pocket = to_interactions[i].shape[0]
                        n_types = to_interactions[i].shape[-1]
                        n_copy = min(n_from_lig, n_to_lig)
                        resized = torch.zeros(
                            n_pocket,
                            n_from_lig,
                            n_types,
                            dtype=to_interactions[i].dtype,
                            device=to_interactions[i].device,
                        )
                        resized[:, :n_copy, :] = to_interactions[i][:, :n_copy, :]
                        from_interactions.append(resized)
                    else:
                        from_interactions.append(to_interactions[i])
                interp_interactions = from_interactions
            else:
